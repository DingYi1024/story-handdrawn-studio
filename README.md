...
> 面向中文故事与有序图片的可扩展手绘动画工作室。功能增删与架构入口见 [CUSTOMIZATION.md](CUSTOMIZATION.md)。
...
```bash
git clone <YOUR_REPOSITORY_URL> story-handdrawn-studio
cd story-handdrawn-studio
npm ci
npm run check      # TypeScript 检查 + 分镜结构校验,不访问网络
npm run check:assets # 生成素材后执行；正式渲染命令也会自动执行
```
...
```bash
# Codex
cp -R skill-package/story-handdrawn-studio ~/.codex/skills/

# Claude Code / 通用 Agent
cp -R skill-package/story-handdrawn-studio ~/.claude/skills/

# Kimi Code
cp -R skill-package/story-handdrawn-studio ~/.agents/skills/
```

3. 告诉 Skill 渲染器项目在哪里(在渲染器项目目录内运行 Agent 时可省略):

```bash
export STORY_HANDDRAWN_STUDIO_PROJECT=/absolute/path/to/story-handdrawn-studio
```
...
```text
使用 $story-handdrawn-studio 把这段故事生成可后期配音的手绘动画。
...
```text
使用 $story-handdrawn-studio 把 /absolute/story.txt 生成手绘动画,标题叫「纸上的夏天」。
```

**上传图片 → 手绘动画**(图片按播放顺序给出):

```text
使用 $story-handdrawn-studio 把这几张图片按顺序生成手绘动画:
...
```text
使用 $story-handdrawn-studio 把这些图片做成翻书效果的手绘动画:
...
```text
使用 $story-handdrawn-studio 先给这个故事生成一个预览版。
```
...
Skill 的完整行为约定见 [skill-package/story-handdrawn-studio/SKILL.md](skill-package/story-handdrawn-studio/SKILL.md)。
...
npm ci
npm run check
npm run check:assets # Run after generating assets; render commands enforce it too
```
...
Final 1080×1440, preview 720×960, H.264, silent. The full behavior contract lives in [SKILL.md](skill-package/story-handdrawn-studio/SKILL.md).
