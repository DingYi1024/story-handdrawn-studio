...
import {validateSettings} from './lib/presets.mjs';
...
const title = String(args.title || studioProject?.title || '上传图片手绘动画');
...
const pageDuration = Number(
  args['page-duration'] || settings?.timing?.uploaded_page_seconds || 4.4,
);
...
const safeTitle = safeSlug(title, 'uploaded').slice(0, 32);
...
const publicAssetRoot = scope
  ? `projects/${scope}/assets/uploads/${assetSet}`
  : `assets/generated/uploads/${assetSet}`;
const outputDir = resolve(publicDir, publicAssetRoot);
...
for (let index = 0; index < inputs.length; index += 1) {
  ...
  const asset = (name) => `${publicAssetRoot}/${name}`;
  scenes.push({
    id,
...
const storyboardPath = resolve(
  root,
  args.storyboard || (workspace ? resolve(workspace, 'storyboard.json') : 'storyboard.uploaded.json'),
);
const manifestPath = resolve(
  root,
  args.manifest || (workspace ? resolve(workspace, 'uploaded-pages.json') : 'uploaded-pages.json'),
);
