...
const assertInside = (base, target, label) => {
  ...
  if (rel.startsWith('..') || isAbsolute(rel)) {
    throw new Error(`${label} escapes ${resolve(base)}: ${absolute}`);
  }
  return absolute;
};
...
const workspaceDir = args.workspace
  ? resolve(root, String(args.workspace))
  : root;
...
const missing = manifest.jobs
  .map((job) => assertInside(publicDir, job.output_master, `Job ${job.id} output_master`))
...
for (const job of manifest.jobs.filter((item) => item.role !== 'reference')) {
  const masterPath = assertInside(publicDir, job.output_master, `Job ${job.id} output_master`);
...
if (args.apply === true) {
  const sourceStoryboard = assertInside(workspaceDir, manifest.storyboard, 'Manifest storyboard');
  const targetStoryboard = assertInside(
    workspaceDir,
    resolve(root, String(args['apply-to'] || 'storyboard.json')),
    'Activation target',
  );
  mkdirSync(dirname(targetStoryboard), {recursive: true});
  copyFileSync(sourceStoryboard, targetStoryboard);
  console.log(`Activated storyboard → ${targetStoryboard}`);
}
